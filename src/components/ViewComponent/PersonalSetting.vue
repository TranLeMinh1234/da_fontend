<template>
    <ItemDropDown
        :config="{
            width: 330,
            height: 340,
            arrowPositionX: 130,
            directArrow: 'top'
        }"
        :isShowDropDown="isShowPersonalSetting"
        @showDropDownEvent="showPersonalSetting"
        @closeDropDownEvent="closePersonalSetting"
    >
        <template #item>
            <img :src="linkImg(userInfo?.fileAvatarName)" alt="" class="avatar-userinfo">
        </template>
        <template #dropdown>
            <div class="personal-info-setting">
                <div class="avatar d-flex center-items">
                    <img :src="linkImg(userInfo?.fileAvatarName)" alt="">
                </div>
                <div class="fw-600 fs-20 w-100 txt-al-center">{{userInfo?.firstName}} {{userInfo?.lastName}}</div>
                <div class="fs-14 w-100 txt-al-center">{{userInfo?.phoneNumber}}</div>
                <div class="fs-14 w-100 txt-al-center">{{userInfo?.email}}</div>
                <div class="setting-feature-item d-flex al-center mg-t-10">
                    <div class="change-password"></div>
                    <div class="pd-l-12">Đổi mật khẩu</div>
                </div>
                <div class="setting-feature-item d-flex al-center">
                    <div class="account-setting"></div>
                    <div class="pd-l-12">Thiết lập tài khoản</div>
                </div>
                <div class="d-flex center-items logout" @click="logOut">
                    <div class="logout-icon"></div>
                    <div>Đăng xuất</div>
                </div>
            </div>
        </template>
    </ItemDropDown>
</template>

<script>
import BaseComponent from '../commonComponent/BaseComponent.vue';
import ItemDropDown from '../commonComponent/ItemDropDown.vue';
import {baseCallApi} from '../../common/js/BaseCallApi.js';

export default {
    name: 'PersonalSetting',
    extends: BaseComponent,
    components: {
        ItemDropDown
    },
    methods: {
        logOut()
        {
            let me = this;
            me.clearToken();
            me.clearUserInfo();
            me.$router.push({name: 'Login'});
        },
        showPersonalSetting()
        {
            let me = this;
            me.isShowPersonalSetting = true;
        },
        closePersonalSetting()
        {
            let me = this;
            me.isShowPersonalSetting = false;
        },
        linkImg(fileName)
        {
            let me = this;
            if(fileName)
            {
                return baseCallApi.doMain + '' + `/file/img/${fileName}`;
            }
            
            return 'data:image/png;base64,';
        },
    },
    data()
    {
        return {
            isShowPersonalSetting: false
        };
    }
}
</script>

<style scoped>
@import url('../../assets/css/ViewComponent/PersonalSetting.css');
</style>